 ...
 ***************************************************************************/
//
//xadreco versão 5.6. Motor de xadrez compatível com o XBoard/WinBoard (C)
  ...
  if (debug)
    {
      fprintf (fsaida,
	       ...
	       version);
      fprintf (fsaida,
	       "Xadreco versão %.1f, Copyright (C) 2004 Ruben Carlo Benante\n"
	       "O Xadreco não possui QUALQUER GARANTIA;\n"
	       "Ele é software livre e você está convidado a redistribui-lo "
	       "sob certas condições; Por favor, visite http://www.fsf.org/licenses/gpl.html\n"
	       "para obter detalhes.\n\n", version);
...
//se existe um resultado, envia ele para finalizar a partida
		...
		msgsai ("Computador sem lances válidos 1", 35);
...
//cloock1 marca o tempo do jogo do adversário. difcloock=cloock2-cloock1. É long int %ld
      difclock = 0;
...
//        printf ("st %d\n", tempomovclock / 100); //esse comando não é da engine para o xboard, mas sim o inverso.
	  tente = 1;
	  continue;
	}
...
//8 é errado, mas é atingido. precisa arrumar isso.
		msgsai ("Posição FEN incorreta.", 24);
	      switch (movinito[k])
//KkQqRrBbNnPp
		{
		case 'K':
...
//              msgsai("Computador sem lances válidos 3", 36);
//algum problema ocorreu que esta sem lances
    return ('e');
...
profsuf (tabuleiro atual, int prof, int alfa, int beta, int niv)
{
  ...
  //retorna sem analisar... Deve desconsiderar o lance. Usuário clicou em "move now" (?)
...
{
  if (!pmovi)
    msgsai ("enche_pmovi precisa de um ponteiro válido", 34);
  pmovi->lance[0] = c0;
//lance em notação inteira
  pmovi->lance[1] = c1;
//move in integer notation
