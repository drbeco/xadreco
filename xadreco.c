...
//            	;
	...
	do
	{
		...
		tabu.roquep=1;
		tabu.peao_pulou=-1; //-1:o adversario nao andou duas com peao.
...
joga_novamente:
		...
		case 'c':
        		    if(xboard)
                    {
			            if(debug)
                        {
    	    				...
    	    				fprintf(fsaida,"xadreco: 1/2-1/2 {both players agree to draw}\n");
                        }
                    	printf("offer draw\n");
		...
		case 'e':
                    ...
                    {
			            if(debug)
	                    	fprintf(fsaida,"xadreco: 1/2-1/2 {internal error aborted engine}\n");
...
void mostra_lances(tabuleiro tabu)
{
    ...
    {
//        res=analisa(&tabu);
//        //nivel tempo pontuação totalnodo variaçao
...
/*    if(!xboard)
    {
		...
		for(i=0;i<2;i++)
		{
			l[2*i]=(int)(m[3*i]-'a');      //l[0]=m[0]-'a'   l[2]=m[3]-'a'
			l[2*i+1]=(int)(m[3*i+1])-'1';  //l[1]=m[1]-'1'   l[3]=m[4]-'1'
		}
    }
    else
    {
...
char humajoga(tabuleiro *tabu) //----------------------------------------------
{
	...
	do
	{
		...
		if(!strcmp(movinito,"hint") && xboard) // !strcmp(movinito,"analyze")
		{
			mostra_lances(*tabu);
            tente=1;
            continue;
...
//				getchar();
		...
		if(!strcmp(movinito,"go") && xboard) //troca de lado. o mesmo que "adv"
        {
//			aux=primeiro;
//         	primeiro=segundo;
//         	segundo=aux;
...
//				scanf("%c",&pausa); //pausa=getche();
				...
				pausa='n';
		//joga quem tem que jogar
//			if(tabu->vez==brancas && primeiro=='c' || tabu->vez==pretas && segundo=='c')
...
//				continue;
//            }
			return('y'); //se for comp. ou humano, gira o laço assim mesmo.
		}
		...
		if(!strcmp(movinito,"ping") || !strcmp(movinito,"setboard")
        || !strcmp(movinito,"playother") || !strcmp(movinito,"san")
...
//	insere_listab(*tabu);
    ...
    if(analise>0 && xboard) // analise outro
    {
        analisa(tabu);
//    	mostra_lances(*tabu);
//            return('y');
...
char compjoga(tabuleiro *tabu)
{
	...
	int i,j, casacor, nv, melhorvalor1;  //nao
	float lan;         //nao
	listab *plaux;     //nao
...
char analisa(tabuleiro *tabu)
{
    tabuleiro tanalise;
    char res, m[80];
    int nv, melhorvalor1; //, melhornv1; // lances calc. em maior nivel tem mais importancia?
...
//mudou para busca em amplitude: variavel nivel obsoleta!
	...
	if(result.plance==NULL)       //...profundidade zero, limite maximo de estatico, limite minimo de estatico
		return('e');		//erro! computador sem lances!? nivel deve ser > zero
    return(' '); //tudo certo...
}
...
//	default:	break;
	...
	//falta explicar que Peca Maior atacada por Peca Menor nao deve ser defendida e sim retirada.
...
//				continue;
//			if(peca<0) //se peca branca:
//				if(tabu.vez==pretas && qataca(brancas,i,j,tabu)<qataca(pretas,i,j,tabu)) //vez das brancas ou defesab>=ataquep
//					totb-=abs(peca);  //pecas brancas: total=total + material. So soma se nao estiver pendurada!
//			else						//se peca preta:
//				if(tabu.vez==brancas && qataca(pretas,i,j,tabu)<qataca(brancas,i,j,tabu)) //vez das pretas ou defesap>=ataqueb
//					totp-=peca;		//pecas pretas: total=total + material. So soma se nao estiver pendurada!
//		}
...
//			if(peca<0) //se peca branca:
//            {
//				if(tabu.vez==pretas || qataca(brancas,i,j,tabu,&menorb)>=qataca(pretas,i,j,tabu,&menorp)) //vez das brancas ou defesab>=ataquep
//					totb+=abs(peca);  //pecas brancas: total=total + material. So soma se nao estiver pendurada!
//            }
//			else						//se peca preta:
//				if(tabu.vez==brancas || qataca(pretas,i,j,tabu,&menorp)>=qataca(brancas,i,j,tabu,&menorb)) //vez das pretas ou defesap>=ataqueb
//					totp+=peca;		//pecas pretas: total=total + material. So soma se nao estiver pendurada!
//		}
...
//            totp-=2*DAMA;
	...
	if(tabu.vez==brancas) // se vez das brancas
		...
		return totp-totb;  // retorna positivo se as brancas estao ganhando (ou negativo c.c.)
}
...
void retira_listab(void)  //retira o plfinal;
{
	listab *plaux;
