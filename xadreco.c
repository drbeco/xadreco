 ...
 ***************************************************************************/
//
//xadreco version 5.1. Chess engine compatible with XBoard/WinBoard Protocol (C)
...
//-----------------------------------------------------------------------------
//
//xadreco vers� 5.1. Motor de xadrez compat�el com o XBoard/WinBoard (C)
//Copyright (C) 2004, Ruben Carlo Benante.
//
//Este programa �software livre; voc�pode redistribu�lo e/ou
//modific�lo sob os termos da Licen� Pblica Geral GNU, conforme
//publicada pela Free Software Foundation; tanto a vers� 2 da
//Licen� como (a seu crit�io) qualquer vers� mais nova.
//
//Este programa �distribu�o na expectativa de ser til, mas SEM
//QUALQUER GARANTIA; sem mesmo a garantia impl�ita de
//COMERCIALIZA�O ou de ADEQUA�O A QUALQUER PROP�ITO EM
//PARTICULAR. Consulte a Licen� Pblica Geral GNU para obter mais
//detalhes.
//
//Voc�deve ter recebido uma c�ia da Licen� Pblica Geral GNU
//junto com este programa; se n�, escreva para a Free Software
...
//Centro de Inform�ica
//Sala de P�-gradua�o 7-1
//Caixa Postal 7851
//CEP 50670-970
//Recife, PE, Brasil
//FAX: +55 (81) 2126-8438
...
//-----------------------------------------------------------------------------
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
//%% Direitos Autorais segundo normas do c�igo-livre: (GNU - GPL)
...
//%% criacao: 10/06/1999
//%% vers� para XBoard/WinBoard: 26/09/04
//%% vers� 5.1, de 13/10/04
...
//numero do movimento que a m�uina pode pedir empate pela primeira vez
//ply that engine can ask first draw
#define MOVE_EMPATE2 88
//numero do movimento que a m�uina pode pedir empate pela segunda vez
//ply that engine can ask second draw
#define QUANTO_EMPATE1 -20
// pede empate se tiver menos que 2 PEOES
//ask for draw if he has less then 2 PAWNS
#define QUANTO_EMPATE2 2
...
//promotion: 4=queen, 5=knight, 6=rook, 7=bishop.
    int             valor_estatico;
//dado um tabuleiro, este �o valor est�ico deste tabuleiro ap� a execu�o deste movimento, no contexto da analise minimax
...
//ponteiro para o pr�imo movimento da lista
//pointer to the next move on the list
}   movimento;

typedef struct resultado
//resultado de uma an�ise de posi�o
...
//usado para armazenar o hist�ico do jogo e comparar posi�es repetidas
...
//how many times this position repeated
    struct listab  *prox;
//ponteiro para o pr�imo da lista
...
//ponteiro para a primeira lista de movimentos de uma seq�cia de n�eis a ser analisadas;
...
char            disc;
//sem uso. vari�el de descarte, para nao sujar o stdin, dic=pega()
...
enum piece_values {
    REI = 1000, DAMA = 100, TORRE = 50, BISPO = 32, CAVALO = 30, PEAO = 10, VAZIA = 0, NULA = -1
};
...
int             xboard = 0;
//1:usando interface gr�ica. 0:somente no prompt
//1:using GUI. 0:only prompt
int             analise = 0;
//-1 para primeira vez. 0 para n� analise. 1 para analise. ("exit" coloca ela como 0 novamente)
//-1 first time. 0 do not analyze. 1 analyze game. ("exit" put it 0 again)
int             setboard = 0;
//-1 para primeira vez. 0 n� edita. 1 edita. Posi�o FEN.
...
//total de nodos analisados em um n�el da �vore
...
const int       brancas = -1;
//pe�s brancas s� negativas
//white pieces are negatives
const int       pretas = 1;
//pe�s pretas s� positivas
...
//mostra na tela informacoes do jogo e an�ises
...
//para inicializar alguns valores no in�io do programa;
//to start some values in the beggining of the program
void            coloca_pecas(tabuleiro * tabu);
//coloca as pe�s na posi�o inicial
...
//limpa algumas vari�eis para iniciar a pondera�o
...
void            msgsai(char *msg);
//emite mensagem antes de sair do programa (por falta de mem�ia ou outros itens), ou tudo certo
...
//retorna o numero de ataques da "cor" na casa(col,lin) de tabu, *menor �a menor pe� que ataca.
...
//retorna o valor est�ico de um tabuleiro ap� jogada a lista de movimentos cabeca
// computador joga ---------------
struct movimento *geramov(tabuleiro tabu);
...
void            minimax(tabuleiro atual, int prof, int uso, int passo, int niv);
//coloca em result a melhor variante e seu valor.
//put in result the better variation and its value
int             profsuf(tabuleiro atual, int prof, int niv);
...
int             estatico(tabuleiro tabu, int cod, int niv);
//retorna um valor estatico que avalia uma posicao do tabuleiro, fixa. Cod==1: tempo estourou no meio da busca. Niv: o nivel de distancia do tabuleiro real para a c�ia examinada
...
//Insere o tabuleiro tabu na lista listab. posiciona plcabeca, plfinal.   Para casos de empate de repeticao de lances, e para pegar o hist�ico de lances
...
int             main(int argc, char *argv[])
{
    ...
    setbuf(stdout, NULL);
    setbuf(stdin, NULL);
    sprintf(feature, "%s%.1f%s", "feature ping=0 setboard=1 playother=1 san=0 usermove=0 time=0 draw=1 sigint=1 sigterm=1 reuse=1 analyze=1 myname=\"Xadreco v.", version, ", by Beco\" variants=\"normal\" colors=0 ics=0 name=0 pause=0");
    if (debug) {
    	ftemp = fopen("xadreco-f.temp", "r");
        if (ftemp==NULL)
//arquivo n� exite? entao �machine 1.
        {
...
//arquivo j�existe? ent� �machine 2.
        {
	    fclose(ftemp);
            machine = 2;
            debug = 1;
//m�uina 2 n� pode fazer debug avan�do.
            fsaida = fopen("xadreco-s.log", "w");
        }
        if (!fsaida)
            debug = 0;
//n� conseguiu abrir nenhum dos dois arquivos. desliga debug!
    }
    printf("Xadreco version %.1f, Copyright (C) 2004 Ruben Carlo Benante\n"
           ...
           "for details.\n\n", version);
    printf("Xadreco vers� %.1f, Copyright (C) 2004 Ruben Carlo Benante\n"
           "O Xadreco n� possui QUALQUER GARANTIA;\n"
           ...
           "para obter detalhes.\n\n", version);
    if (debug) {
        fprintf(fsaida, "Xadreco version %.1f, Copyright (C) 2004 Ruben Carlo Benante\n"
                "Xadreco comes with ABSOLUTELY NO WARRANTY;\n"
                "This is free software, and you are welcome to redistribute it "
                "under certain conditions; Please, visit http://www.fsf.org/licenses/gpl.html\n for details.\n\n", version);
        fprintf(fsaida, "Xadreco vers� %.1f, Copyright (C) 2004 Ruben Carlo Benante\n"
                "O Xadreco n� possui QUALQUER GARANTIA;\n"
                "Ele �software livre e voc�est�convidado a redistribui-lo "
                "sob certas condi�es; Por favor, visite http://www.fsf.org/licenses/gpl.html\n"
                "para obter detalhes.\n\n", version);
        if (machine == 1)
            fprintf(fsaida, "xadreco : primeira m�uina\n");
        else if (machine == 2)
            fprintf(fsaida, "xadreco : segunda m�uina\n");
    }
    if (argc > 1)
...
//aceitar empate
        ...
        case 'i':
            if (xboard) {
                strcpy(ultimo_resultado, "1/2-1/2 {insufficient mating material}");
                if (debug)
...
//Inclui o caso de Rei x Rei e Pecas: obrigatorio dar mate em 50 lances
        ...
        case 'r':
            if (xboard) {
                ...
                if (debug)
                    fprintf(fsaida, "xadreco : 1/2-1/2 {Draw by triple repetition}\n");
...
//se existe um resultado, envia ele para finalizar a partida
                {
                    ...
                    msgsai("Computador sem lances v�idos 1");
//algum problema ocorreu que esta sem lances
                primeiro = 'h';
                segundo = 'h';
                goto joga_novamente;
            } else {
                printf("\nSem movimentos v�idos para fazer a partir deste ponto!");
                break;
            }
        case 'w':
            opc = 's';
...
//tabu.situa=2
        ...
        case 'y':
//retorna y: simplesmente gira o la� para jogar de novo. Troca de adv.
        default:
//'-' para tudo certo...
...
//vai para tela
        ...
        msgsai("quit\n");
//vai apenas para o log, mas n� para a sa�a
    return 0;
}
...
//nivel est�errado!
        printf("%d %d %d %d ", nivel, result.valor * 10, difclock / 10, totalnodo);
//result.valor*10 o Winboard divide por 100.
        if (debug)
            fprintf(fsaida, "xadreco : %d %+.2f %d %d ", nivel, (float) result.valor / 10.0, difclock / 10, totalnodo);
//result.valor/10 para a Dama ficar com valor 10
        imprime_linha(result.plance, tabu.numero, tabu.vez);
        printf("\n");
        if (debug)
            fprintf(fsaida, "\n");
...
//nao eh xboard.
    if (tabu.lancex[0] == 0 && tabu.lancex[1] == 0 && tabu.lancex[2] == 0 && tabu.lancex[3] == 0) {
        if (!xboard)
...
//imprime o movimento
        if (!xboard)
            if (tabu.vez == pretas)
                printf("Lance executado :%d. %s", (int) ((tabu.numero + 1.0) / 2.0), movinito);
            else
                printf("Lance executado :%d. ... %s", (int) ((tabu.numero + 1.0) / 2.0), movinito);
        else {
...
//as coordenadas origem e destino estao no tabuleiro
        if (m[2 * i] < 'a' || m[2 * i] > 'h' || m[2 * i + 1] < '1' || m[2 * i + 1] > '8')
//2i e 2i+1
            return (0);
    for (i = 0; i < 2; i++) {
        l[2 * i] = (int) (m[2 * i] - 'a');
...
//enche_pmovi(pmovi, c0, c1, c2, c3, peao, roque, espec, flag);
                                    ...
                                    pmovi->prox = (movimento *) malloc(sizeof(movimento));
                                    if (pmovi->prox == NULL)
                                        msgsai("Erro de alocacao de memoria em geramov 3");
                                    pmoviant = pmovi;
                                    pmovi = pmovi->prox;
                                    pmovi->prox = NULL;
                                }
...
//preenche a estrutura movimento
                                    pmovi->prox = (movimento *) malloc(sizeof(movimento));
                                    if (pmovi->prox == NULL)
                                        msgsai("Erro de alocacao de memoria em geramov 4");
                                    pmoviant = pmovi;
                                    pmovi = pmovi->prox;
                                    pmovi->prox = NULL;
                                }
...
//Nao mexeu TR (e a torre n� foi capturada)
                    {
// roque pequeno
...
//preenche a estrutura movimento
                                pmovi->prox = (movimento *) malloc(sizeof(movimento));
                                if (pmovi->prox == NULL)
                                    msgsai("Erro de alocacao de memoria em geramov 5");
                                pmoviant = pmovi;
                                pmovi = pmovi->prox;
                                pmovi->prox = NULL;
                            }
...
//preenche a estrutura movimento
                                pmovi->prox = (movimento *) malloc(sizeof(movimento));
                                if (pmovi->prox == NULL)
                                    msgsai("Erro de alocacao de memoria em geramov 6");
                                pmoviant = pmovi;
                                pmovi = pmovi->prox;
                                pmovi->prox = NULL;
                            }
...
//preenche a estrutura movimento
                                    pmovi->prox = (movimento *) malloc(sizeof(movimento));
                                    if (pmovi->prox == NULL)
                                        msgsai("Erro de alocacao de memoria em geramov 7");
                                    pmoviant = pmovi;
                                    pmovi = pmovi->prox;
                                    pmovi->prox = NULL;
                                }
...
//preenche a estrutura movimento
                                    pmovi->prox = (movimento *) malloc(sizeof(movimento));
                                    if (pmovi->prox == NULL)
                                        msgsai("Erro de alocacao de memoria em geramov 8");
                                    pmoviant = pmovi;
                                    pmovi = pmovi->prox;
                                    pmovi->prox = NULL;
                                }
...
//se promoveu
                                {
                                    ...
                                    pmovi->prox = (movimento *) malloc(sizeof(movimento));
                                    if (pmovi->prox == NULL)
                                        msgsai("Erro de alocacao de memoria em geramov 9");
                                    pmoviant = pmovi;
                                    pmovi = pmovi->prox;
                                    pmovi->prox = NULL;
                                    if (k > 3)
...
//nao promoveu
                                {
                                    ...
                                    pmovi->prox = (movimento *) malloc(sizeof(movimento));
                                    if (pmovi->prox == NULL)
                                        msgsai("Erro de alocacao de memoria em geramov 10");
                                    pmoviant = pmovi;
                                    pmovi = pmovi->prox;
                                    pmovi->prox = NULL;
                                }
...
//se promoveu
                                {
                                    ...
                                    pmovi->prox = (movimento *) malloc(sizeof(movimento));
                                    if (pmovi->prox == NULL)
                                        msgsai("Erro de alocacao de memoria em geramov 11");
                                    pmoviant = pmovi;
                                    pmovi = pmovi->prox;
                                    pmovi->prox = NULL;
                                    if (k > 3)
...
//nao promoveu
                                {
                                    ...
                                    pmovi->prox = (movimento *) malloc(sizeof(movimento));
                                    if (pmovi->prox == NULL)
                                        msgsai("Erro de alocacao de memoria em geramov 12");
                                    pmoviant = pmovi;
                                    pmovi = pmovi->prox;
                                    pmovi->prox = NULL;
                                }
...
//preenche a estrutura movimento
                                ...
                                pmovi->prox = (movimento *) malloc(sizeof(movimento));
                                if (pmovi->prox == NULL)
                                    msgsai("Erro de alocacao de memoria em geramov 13");
                                pmoviant = pmovi;
                                pmovi = pmovi->prox;
                                pmovi->prox = NULL;
                            }
...
//preenche a estrutura movimento
                                ...
                                pmovi->prox = (movimento *) malloc(sizeof(movimento));
                                if (pmovi->prox == NULL)
                                    msgsai("Erro de alocacao de memoria em geramov 14");
                                pmoviant = pmovi;
                                pmovi = pmovi->prox;
                                pmovi->prox = NULL;
                            }
...
//4,5,6,7 == D,C,T,Bispo
                                    ...
                                    pmovi->prox = (movimento *) malloc(sizeof(movimento));
                                    if (pmovi->prox == NULL)
                                        msgsai("Erro de alocacao de memoria em geramov 15");
                                    pmoviant = pmovi;
                                    pmovi = pmovi->prox;
                                    pmovi->prox = NULL;
                                    if (l > 3)
...
//comeu sem promover
                                {
                                    ...
                                    pmovi->prox = (movimento *) malloc(sizeof(movimento));
                                    if (pmovi->prox == NULL)
                                        msgsai("Erro de alocacao de memoria em geramov 16");
                                    pmoviant = pmovi;
                                    pmovi = pmovi->prox;
                                    pmovi->prox = NULL;
                                }
...
//4,5,6,7 == D,C,T,Bispo
                                    ...
                                    pmovi->prox = (movimento *) malloc(sizeof(movimento));
                                    if (pmovi->prox == NULL)
                                        msgsai("Erro de alocacao de memoria em geramov 17");
                                    pmoviant = pmovi;
                                    pmovi = pmovi->prox;
                                    pmovi->prox = NULL;
                                    if (l > 3)
...
//comeu sem promover
                                {
                                    ...
                                    pmovi->prox = (movimento *) malloc(sizeof(movimento));
                                    if (pmovi->prox == NULL)
                                        msgsai("Erro de alocacao de memoria em geramov 18");
                                    pmoviant = pmovi;
                                    pmovi = pmovi->prox;
                                    pmovi->prox = NULL;
                                }
...
//m=idem para a vertical
                        do {
                            if (col >= 0 && col <= 7 && sinal(tabu.tab[col][j]) != tabu.vez && l == 0)
//gira col, mantem lin
                            {
...
//Dama ou Torre capturou peca adversaria.
                                    ...
                                    pmovi->prox = (movimento *) malloc(sizeof(movimento));
                                    if (pmovi->prox == NULL)
                                        msgsai("Erro de alocacao de memoria em geramov 20");
                                    pmoviant = pmovi;
                                    pmovi = pmovi->prox;
                                    pmovi->prox = NULL;
                                } else
...
//nao inclui mais nenhuma casa desta linha; A casa esta fora do tabuleiro, ou tem peca de mesma cor ou capturou
                                l = 1;
                            if (lin >= 0 && lin <= 7 && sinal(tabu.tab[i][lin]) != tabu.vez && m == 0)
//gira lin, mantem col
                            {
//inclui esta casa na lista
                                copitab(&tabaux, &tabu);
...
//Dama ou Torre capturou peca adversaria.
                                    ...
                                    pmovi->prox = (movimento *) malloc(sizeof(movimento));
                                    if (pmovi->prox == NULL)
                                        msgsai("Erro de alocacao de memoria em geramov 21");
                                    pmoviant = pmovi;
                                    pmovi = pmovi->prox;
                                    pmovi->prox = NULL;
                                } else
...
//Dama ou Bispo capturou peca adversaria.
                                    ...
                                    pmovi->prox = (movimento *) malloc(sizeof(movimento));
                                    if (pmovi->prox == NULL)
                                        msgsai("Erro de alocacao de memoria em geramov 22");
                                    pmoviant = pmovi;
                                    pmovi = pmovi->prox;
                                    pmovi->prox = NULL;
                                } else
...
// achou peca, mas esta nao anda em diagonal ou e' peca propria
                else if (tabu.tab[casacol][casalin] == BISPO || tabu.tab[casacol][casalin] == DAMA)
                    return (1);
        }
// proxima diagonal
// ataque de rei...
...
//para testar uma posi�o
    ...
    do {
        ...
        if (!xboard)
            printf("\n\nComando: ");
        scanf("%s", movinito);
//---------------- (*quase) Toda entrada do Winboard
        clock1 = clock();
//clock1 marca o tempo do jogo do advers�io. difclock=clock2-clock1. �long int %ld
        difclock = 0;
        fflush(fsaida);
//grava o arquivo atual, para o caso de bug, poder ler algo
        if (debug)
...
// analise termina
        {
            analise = 0;
//            testasim=0; nao �necess�ia, pois �vari�el local.
            tente = 1;
            continue;
        }
...
//Se o computador (positivo) < Limite QUANTO_EMPATE1, ent� ele aceita.
//O detalhe �que o computador s�aceita se ele estiver perdendo dois pe�s.
...
//so se perdendo 2 pe�s (e n� est�jogando contra outra engine)
                {
                    if (debug)
                        fprintf(fsaida, "xadreco : draw accepted (HumaJ-1) %d  turn: %d\n", estat, tabu->vez);
                    return ('c');
                } else {
                    if (debug)
        ...
        if (!strcmp(movinito, "versao") || !strcmp(movinito, "version")) {
            if (!xboard) {
                printf("\nXadreco vers� %.1f para XBoard/WinBoard", version);
                disc = pega(" ", "\nBaseado no Algoritmo Minimax.\n\n		Por Ruben Carlo Benante, 22/10/04.\nTecle <ESPACO>+<ENTER>");
                imptab(*tabu);
            } else {
                if (debug)
                    fprintf(fsaida, "tellopponent Xadreco v.%.1f para XBoard/WinBoard, baseado no Algoritmo Minimax, por Ruben Carlo Benante, 22/10/04.\n", version);
                printf("tellopponent Xadreco v.%.1f para XBoard/WinBoard, baseado no Algoritmo Minimax, por Ruben Carlo Benante, 22/10/04.\n", version);
            }
            tente = 1;
            continue;
        }
        if (!strcmp(movinito, "sd") && xboard)
//set deep: coloca o n�el de profundidade. Nao est�sendo usado. Implementar.
        {
        ...
        if (!strcmp(movinito, "nivel") && !xboard) {
            ...
            if (incre < 0)
                incre = 0;
            tempomov = minutes * 60000 / moves + incre * 1000;
//para campeonato, tem que jogar mais r�ido!
            imptab(*tabu);
            tente = 1;
            continue;
        }
...
//coloca o computador com a cor que n� �a da vez.
        {
            if (tabu->vez == brancas) {
                primeiro = 'h';
                segundo = 'c';
...
//n� abandona nunca
        ...
        if (!strcmp(movinito, "adv") && !xboard) {
            ...
            if (primeiro == segundo && primeiro == 'c')
                ...
                pausa = 'n';
            return ('y');
//se for comp. ou humano, gira o la� e joga quem for.
        }
...
//brancas abandonam
        if (!strcmp(movinito, "quit"))
            msgsai("Obrigado por jogar com o Xadreco.");
...
//= 3000 milisec per mov
        if (!strcmp(movinito, "level") && xboard) {
            ...
            tempomov = minutes * 60000 / moves + incre * 1000;
//            tempomov/=2;
//apesar de correto, est�gastando o dobro do tempo. Problema com as rotinas de apoio? Problema com o minimax que n� sabe retornar?
            if (debug)
...
//Posi�o FEN
            scanf("%s", movinito);
//trava se colocar uma posi�o FEN inv�ida!
            if (!strcmp(movinito, "testpos")) {
                testasim = 1;
//a posi�o vem da fun�o testapos, e n� de scanf()
...
//Veja o que o sono faz no c�igo da gente: (apertando ctrl-s para salvar... ainda bem que n� foi ctrl-z)
//sssssssssss
            else
                scanf("%s", movinito);
            if (debug)
...
// Baseado somente nos movimentos, nao na posicao.
        ...
        if (!strcmp(movinito, "ping") || !strcmp(movinito, "san")
            ...
            || !strcmp(movinito, "{Stalemate}") || !strcmp(movinito, "resigns}")
            || !strcmp(movinito, "1-0") || !strcmp(movinito, "0-1")
            || !strcmp(movinito, "1/2-1/2") || !strcmp(movinito, "{Forfeit")
            || !strcmp(movinito, "due") || !strcmp(movinito, "to")
            || !strcmp(movinito, "illegal") || !strcmp(movinito, "move}")
            || !strcmp(movinito, "{both") || !strcmp(movinito, "players")
            || !strcmp(movinito, "agreed") || !strcmp(movinito, "draw}")
            || !strcmp(movinito, "*") || !strcmp(movinito, "{Game")
            || !strcmp(movinito, "was") || !strcmp(movinito, "unfinished}")) {
            tente = 1;
            continue;
        }
...
//posicao repetiu 3 vezes
    ...
    cabeca = geramov(tabu);
//criar funcao gera1mov() retorna verdadeiro ou falso
    if (cabeca == NULL)
    ...
    if (pausa == 'c' && !xboard) {
        do {
            ...
            if (!strcmp(command, "quit"))
                msgsai("Obrigado por jogar com o Xadreco.");
            else if (!strcmp(command, "desisto") || !strcmp(command, "resign"))
                if (tabu->vez == brancas)
                    return ('B');
                else
                    return ('b');
...
//comando repete
                ...
                printf("\nOs comandos sao:\n");
                printf("	lista  : mostra os movimentos possiveis e o melhor lance.\n");
                printf("	desisto: determina o fim deste jogo, com abandono para a cor da vez.\n");
                printf("	quit   : termino anormal do programa.\n");
                printf("	repete : mostra a lista de tabuleiros.\n");
                printf("	nivel  : muda o nivel de dificuldade atual (tempo do jogo).\n");
                com = pega("c ", "\nTecle <ESPACO>+<ENTER> para prosseguir ou <c>+<ENTER> para comando");
            }
        }
        while (com == 'c');
...
//nivel extra de debug
            {
                fprintf(fmini, "\n\n*************************************************************");
                fprintf(fmini, "\nminimax(*tabu, prof=0, uso=%d, passo=%d, nv=%d)", LIMITE, -LIMITE, nv);
            }
...
//retorna o melhor caminho a partir de tab...
            if (result.plance == NULL)
//Nova defini�o: sem lances, pode ser que queira avan�r ap� mate.
                break;
            if ((long int)difclock < tempomov)
//18/10/2004, 19:20 +3 GMT. Descobri e criei esse teste ap� muito sofrimento em debugs. Fim da vers� 5.1!
            {
                libera_lances(melhorcaminho1);
                melhorcaminho1 = copilistmov(result.plance);
                melhorvalor1 = result.valor;
...
//ordena succ_geral
            if (debug == 2) {
                fprintf(fmini, "\nresult.valor: %d totalnodo: %d\nresult.plance: ", result.valor, totalnodo);
...
//nivel tempo valor lances no arena. (nao usado aqui. testar)
        if (mostrapensando && abs(result.valor)!=FIMTEMPO) {
            printf("%d %d %d %d ", nv, result.valor * 10, difclock / 10, totalnodo);
            if (debug)
                fprintf(fsaida, "xadreco : %d %+.2f %d %d ", nv, (float) result.valor / 10.0, difclock / 10, totalnodo);
            imprime_linha(result.plance, tabu->numero + 1, -tabu->vez);
            printf("\n");
            if (debug)
                fprintf(fsaida, "\n");
        }
// termino do la� infinito baseado no tempo
            if ((long int)difclock > tempomov && debug!=2 || (debug == 2 && nv == 3))
// termino do la� infinito baseado no tempo
                break;
            nv++;
        }
    }
// fim do se n� usou livro
...
//Utilizado: ABANDONA==-273, alterado quando contra outra engine
    {
        if (debug)
...
//pode oferecer empate duas vezes. Uma assim que perder 2 pe�s. Outra ap� 60 lances e com menos de 2 pe�s
        ofereci = 1;
...
//oferecer empate: result.valor est�invertido na vez.
    {
        if (debug)
            fprintf(fsaida, "xadreco : offer draw (1) value: %d\n", result.valor);
        --ofereci;
        printf("offer draw\n");
//aten�o: oferecer pode significar aceitar, se for feito logo ap� uma oferta recebida.
    }
    if (result.valor < QUANTO_EMPATE2 && tabu->numero >= MOVE_EMPATE2 && ofereci > 0)
//oferecer empate: result.valor est�invertido na vez.
    {
        if (debug)
            fprintf(fsaida, "xadreco : offer draw (2) value: %d\n", result.valor);
        --ofereci;
        printf("offer draw\n");
//aten�o: oferecer pode significar aceitar, se for feito logo ap� uma oferta recebida.
    }
...
// joga_em j�usa insere_listab(*tabu);
//insere o lance jogado na lista de repeteco
    return (res);
...
//mudou para busca em amplitude: variavel nivel obsoleta!
    ...
    if (result.plance != NULL) {
        ...
        difclock = clock2 - clock1;
//nivel pontua�o tempo totalnodo varia�o === usado!
        printf("%d %d %d %d Livro: ", nv, result.valor * 10, difclock / 10, totalnodo);
        if (debug)
            fprintf(fsaida, "xadreco : %d %+.2f %d %d Livro: ", nv, (float) result.valor / 10.0, difclock / 10, totalnodo);
        imprime_linha(result.plance, tabu->numero + 1, -tabu->vez);
        printf("\n");
        if (debug)
            fprintf(fsaida, "\n");
...
//nivel extra de debug
        (void)fclose(fmini);
    if (result.plance == NULL)
//...ap� o t�mino da partida, s�pode-se usar edi�o, undo, etc.
//              msgsai("Computador sem lances v�idos 3");
//algum problema ocorreu que esta sem lances
        return ('e');
//erro! computador sem lances!? nivel deve ser > zero
    return ('-');
...
//coloque um nulo no ponteiro plance
//nao eh necessario libera_lance, pois plance eh tempor�io apesar de global.
        result.valor = estatico(atual, 0, prof);
//estatico(tabuleiro, 1: acabou o tempo, 0: nao acabou. prof: qual nivel est�analisando?)
        if (debug == 2)
            fprintf(fmini, "NULL ");
//! result.valor=%d", result.valor);
        return;
    }
    cabeca_succ = succ;
    while (succ != NULL)
//laco para analisar todos sucessores
    {
        copitab(&tab, &atual);
...
//tab.situa: 1=Empate, 3,4=Mate ou 5,6=Tempo. 7=sem resultado. Nao pode passar o nivel
        ...
        minimax(tab, prof + 1, -passo, -uso, niv);
...
//implementar o NULL-MOVE
        if (passo >= uso)
//corte alfa-beta! Isso est�certo? N� �passo<=uso? Conferir. (alfa==passo, beta==uso, para brancas)
        {
            if (debug == 2) {
                lance2movi(m, succ->lance, succ->especial);
                fprintf(fmini, "\nsucc: passo>=uso (%d>=%d) %s (%d%d%d%d) Corte Alfa/Beta!", passo, uso, m, succ->lance[0], succ->lance[1], succ->lance[2], succ->lance[3]);
            }
            break;
//faz o corte!
        }
...
int             profsuf(tabuleiro atual, int prof, int niv)
{
    clock2 = clock();
    difclock = clock2 - clock1;
//tem que analisar pelo menos 15 miliseg cada 50 nodos
//liberou, aprofunda.
...
//se ja passou do nivel estipulado, pare a busca incondicionalmente
    if (prof > niv)
//se est�ou ja passou do nivel estipulado, pare a busca incondicionalmente
    {
//        libera_lances(result.plance); //bugbug
        result.plance = NULL;
//coloque um nulo no ponteiro plance
...
//estatico(tabuleiro, 1: acabou o tempo, 0: nao acabou. Prof: qual nivel est�analisando?)
    ...
    if ((long int)difclock > tempomov && debug!=2)
//retorna sem analisar... Deve desconsiderar o lance
    {
//        libera_lances(result.plance); //bugbug
        result.plance = NULL;
//coloque um nulo no ponteiro plance
//nao eh necessario libera_lance, pois plance eh tempor�io apesar de global.
        result.valor = -FIMTEMPO;//estatico(atual, 1, prof);
//estatico(tabuleiro, 1: acabou o tempo, 0: nao acabou. Prof: qual nivel est�analisando?)
        return 1;
    }
    if (!profflag)
...
//mexeu torre da dama
    ...
    tabu->tab[movi.lance[2]][movi.lance[3]] = tabu->tab[movi.lance[0]][movi.lance[1]];
    tabu->tab[movi.lance[0]][movi.lance[1]] = VAZIA;
    for (i = 0; i < 4; i++)
        tabu->lancex[i] = movi.lance[i];
    tabu->numero++;
//conta os movimentos de cada pe� (�chamado de dentro de fun�o recursiva!)
...
//retorna o valor t�ico e estrat�ico de um tabuleiro, sendo valor positivo melhor quem est�na vez
//cod: 1: acabou o tempo, 0: ou eh avalia�o normal?
//niv: qual a dist�cia do tabuleiro real para a c�ia tabu avaliada?
int             estatico(tabuleiro tabu, int cod, int niv)
{
    int             totb = 0,
                    totp = 0;
...
// Quando est�avaliando uma posi�o e o tempo estourou, por
// via das dvidas �melhor n� confiar muito nessa posi�o...
//------------------------------------------------------------
//ignorando a avalia�o est�ica, pois precisa retornar
    if (cod)
//avaliando posicao estatica e o tempo estourou!
//      if(tabu.vez==brancas)
//nao importa a cor;
        return -FIMTEMPO; //bugbug
...
//se uma pe� preta j�comeu, ela n� pode contar de novo como atacante! (corrigir)
            if (ordem[k][5] == 0)
                continue;
//            qtdb = qataca(brancas, i, j, tabu, &menorb);
//defesas
            if (ordem[k][5] > ordem[k][3])
                totp+=(ordem[k][5]-ordem[k][3]);
            else
                totb+=(ordem[k][3]-ordem[k][5]+1);
            if (ordem[k][6] < -peca)
                totp+=5;
        }
         else {
...
//            qtdp = qataca(pretas, i, j, tabu, &menorp);
            if (ordem[k][3] > ordem[k][5])
                totb+=(ordem[k][3]-ordem[k][5]);
            else
                totp+=(ordem[k][5]-ordem[k][3]+1);
            if (ordem[k][4] < peca)
                totb+=5;
        }
    }
...
//            continue;
        tabu.tab[i][j]=VAZIA;
//imagina se essa pe� n� existisse?
        if(peca<0) {
            qtdp = qataca(pretas, i, j, tabu, &menorp);
...
//hauhauhahauhah! Bonifica�o para quem "perdeu"! o roque na
//abertura!
...
//posi�o inicial
    ...
    {
        {
            {-50, -10, 0, 0, 0, 0, 10, 50, NULA, NULA},
            {-30, -10, 0, 0, 0, 0, 10, 30, NULA, NULA},
            {-32, -10, 0, 0, 0, 0, 10, 32, NULA, NULA},
            {-100, -10, 0, 0, 0, 0, 10, 100, NULA, NULA},
            {-1000, -10, 0, 0, 0, 0, 10, 1000, NULA, NULA},
            {-32, -10, 0, 0, 0, 0, 10, 32, NULA, NULA},
            {-30, -10, 0, 0, 0, 0, 10, 30, NULA, NULA},
            {-50, -10, 0, 0, 0, 0, 10, 50, NULA, NULA}
        },
...
//a funcao joga_em deve inserir no listab, cod: 1:insere, 0:nao insere
        if (n / 5 >= numero)
//chegou na posi�o atual! come� inserir na lista
        {
            if (cabeca == NULL) {
                cabeca = pmovi;
                pmoviant = pmovi;
...
//maximo de linhas no livro!
            ...
            while (linha[0] == '#')
//a ultima linha n� deve ser coment�io
                (void)fgets(linha, 256, flivro);
...
//                              cabeca=string2pmovi(tabu.numero, linha);
//                        }
// Ordenar por valor est�ico da variante
                        libera_lances(cabeca);
                        cabeca = string2pmovi(tabu.numero, linha);
//eh a primeira linha que casou
...
void            inicia(tabuleiro * tabu)
//inicializa vari�eis do programa. (new game)
{
    int             i,
                    j;
    pausa = 'n';
...
void            limpa_pensa(void)
//limpa algumas vari�eis para iniciar pondera�o
{
    libera_lances(result.plance);
    result.plance = NULL;
//eh necessario
...
{
    if (!pmovi)
        msgsai("enche_pmovi precisa de um ponteiro v�ido");
    pmovi->lance[0] = c0;
//lance em nota�o inteira
    pmovi->lance[1] = c1;
//move in integer notation
...
//cuidado para n� estourar o limite da cadeia, teclando muito <t>
...
//2 bispos... mate em 1, ou em 2, ou em 3... Se comer pe�, afoga
//      char *p1="k7/2K5/1P6/2B5/3p4/8/8/8 w - - 0 1";
//mate de peao em 1 ply ou em 3 ply. se comer pe�, afoga
...
//    char *p1="6k1/p2rnppp/Rp1pr3/1Pp1p1PP/2P1P1K1/2RQ4/B4P2/4N3 w - - 50 80";
	...
	char *p1="8/3p4/4k3/8/4P3/2K5/2P5/BB6 w - - 50 1";
    sscanf(p1, "%s %s %s %s %s %s", pieces, color, castle, enpassant, halfmove, fullmove);
}
...
// jogou anterior as brancas
        {
            if (tabuvez == 2)
//codigo so para log, n� imprimir na tela.
            {
                if (fimprime != NULL)
                    fprintf(fimprime, "%s ", m);
            } else
...
//insere pior na cabe� (a lista  fica em ordem decrescente: o melhor na cabe�)
...
//a funcao joga_em deve inserir no listab, cod: 1:insere, 0:nao insere
//com isso surge o problema de que o programa n� detecta empate por 3 repeti�es enquanto l�o livro de aberturas.
//a solu�o simples � n� incluir no livro de abertura linhas que empatam com tr� repeti�es na fase de abertura (se �que isso existe!)
        cabeca = cabeca->prox;
        niv++;
    }
