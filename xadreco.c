//xadreco version 2.0. Chess engine compatible with XBoard/WinBoard Protocol (C)
...
//-----------------------------------------------------------------------------
//
//xadreco version 2.0. Motor de xadrez compatível com o XBoard/WinBoard (C)
//Copyright (C) 2004, Ruben Carlo Benante.
//
...
para obter detalhes.\n\n");
  ...
  if (debug)
    {
      fprintf (fsaida,
	       "Xadreco version 2.0, Copyright (C) 2004 Ruben Carlo Benante\n\
Xadreco comes with ABSOLUTELY NO WARRANTY;\n\
This is free software, and you are welcome to redistribute it \
under certain conditions; Please, visit http://www.fsf.org/licenses/gpl.html\n for details.\n\n");

      fprintf (fsaida,
	       "Xadreco versão 2.0, Copyright (C) 2004 Ruben Carlo Benante\n\
O Xadreco não possui QUALQUER GARANTIA;\n\
Ele é software livre e você está convidado a redistribui-lo \
sob certas condições; Por favor, visite http://www.fsf.org/licenses/gpl.html\n\
para obter detalhes.\n\n");
	...
	if (debug)
	  {
	    ...
	    fprintf (fsaida,
		     "xadreco: feature ping=0 setboard=0 playother=0 san=0 usermove=0 time=0 draw=1 sigint=1 sigterm=1 reuse=1 analyze=1 myname=\"Xadreco v.2, by Beco\" variants=\"normal\" colors=0 ics=0 name=0 pause=0\n");
	    fprintf (fsaida, "xadreco: feature done=1\n");
	  }
	printf ("feature done=0\n");
	printf
	  ("feature ping=0 setboard=0 playother=0 san=0 usermove=0 time=0 draw=1 sigint=1 sigterm=1 reuse=1 analyze=1 myname=\"Xadreco v.2, by Beco\" variants=\"normal\" colors=0 ics=0 name=0 pause=0\n");
...
//        difclock/=10;
      ...
      if (debug)
	fprintf (fsaida, "xadreco: %d %d %d %d ", nivel, result.valor,
		 difclock / 10, totalnodo);
      loop = result.plance;
      while (loop != NULL)
	{
	  lance2movi (m, loop->lance, loop->flag_50, loop->especial);
      ...
      if (!strcmp (movinito, "versao") || !strcmp (movinito, "version"))
	{
	  if (!xboard)
	  ...
	  else
	    {
	      if (debug)
		fprintf (fsaida,
			 "tellopponent Xadreco v.2 para XBoard/WinBoard, baseado no Algoritmo Minimax, por Ruben Carlo Benante, 04/06/04.\n");
	      printf
		("tellopponent Xadreco v.2 para XBoard/WinBoard, baseado no Algoritmo Minimax, por Ruben Carlo Benante, 04/06/04.\n");
	    }
	  tente = 1;
...
minimax (tabuleiro atual, int prof, int uso, int passo, int niv)
{
  ...
  while (succ != NULL)		//laco para analisar todos sucessores
	...
	default:
	  profflag = 0;		//1:Empate, 3,4:Mate ou 5,6:Tempo. Nao pode passar o nivel
	}
...
int
estatico (tabuleiro tabu, int cod)
{
  int totb = 0, totp = 0;
  int i, j, cor, peca;
...
//              }
  ...
  if (tabu.numero < 16)
    {
      if (tabu.tab[6][0] == -REI && tabu.tab[5][1] == tabu.tab[6][1] == tabu.tab[7][1] == -PEAO && tabu.tab[7][0] != -TORRE)	//brancas com roque pequeno
	totb += 5;
      if (tabu.tab[2][0] == -REI && tabu.tab[1][1] == tabu.tab[2][1] == -PEAO)	//brancas com roque grande
	totb += 3;
      if (tabu.tab[6][7] == REI && tabu.tab[5][6] == tabu.tab[6][6] == tabu.tab[7][6] == PEAO && tabu.tab[7][7] != TORRE)	//pretas com roque pequeno
	totp += 5;
      if (tabu.tab[2][7] == REI && tabu.tab[1][6] == tabu.tab[2][6] == PEAO)	//pretas com roque grande
	totp += 3;
    }
