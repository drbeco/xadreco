//xadreco version 4.1. Chess engine compatible with XBoard/WinBoard Protocol (C)
...
//-----------------------------------------------------------------------------
//
//xadreco versão 4.1. Motor de xadrez compatível com o XBoard/WinBoard (C)
//Copyright (C) 2004, Ruben Carlo Benante.
//
...
FILE *fsaida, *ftemp;           //fsaida: arquivo de log xadreco.log, ftemp: arquivo de trava
...
const float version=4.1; //Versão do programa
                            //program version
...
int main(int argc, char *argv[])
{
    ...
    if(debug)
    {
    	...
    	if(!ftemp) //arquivo não exite? entao é machine 1.
	    {
        ...
        else //arquivo já existe? leia numero.
        {
...
para obter detalhes.\n\n",version);
	...
	do
	{
		...
		tabu.tab[0][0]=tabu.tab[7][0]=-TORRE;		tabu.tab[0][7]=tabu.tab[7][7]=TORRE;
		tabu.tab[1][0]=tabu.tab[6][0]=-CAVALO;		tabu.tab[1][7]=tabu.tab[6][7]=CAVALO;
		tabu.tab[2][0]=tabu.tab[5][0]=-BISPO;		tabu.tab[2][7]=tabu.tab[5][7]=BISPO;
		tabu.tab[3][0]=-DAMA; tabu.tab[4][0]=-REI;	tabu.tab[3][7]=DAMA; tabu.tab[4][7]=REI;
		insere_listab(tabu);
//--------------------------------------------------------------------------------
		if(xboard)
        {
		...
		case 'e':
        		    if(xboard)
                    {
                    	if(ultimo_resultado[0]!='\0')
                        {
                        	...
                        	sai(); //algum problema ocorreu que esta sem lances
                        primeiro='h';
                        segundo='h';
						goto joga_novamente;
                    }
...
//        res=analisa(&tabu);
//        difclock/=10;
        //nivel pontuação tempo totalnodo variação
        printf("%d %d %d %d ", nivel, result.valor, difclock/10, totalnodo);
        if(debug)
            fprintf(fsaida,"xadreco : %d %d %d %d ", nivel, result.valor, difclock/10, totalnodo);
		loop=result.plance;
		while(loop!=NULL)
		{
			lance2movi(m,loop->lance,loop->flag_50,loop->especial);
...
if(!xboard)
{
	linmin=7;linmax=-1;inc=-1;
	colmin=0;colmax=8;
	if(primeiro=='c' && segundo=='h')
	{
		linmin=0;linmax=8;inc=1;
		colmin=7;colmax=-1;
	}
...
//					textcolor(0);     //peca positiva: preta.
			...
			case DAMA:		printf(" d "); break;
			case REI:		printf(" r "); break;
// pecas brancas
			case -PEAO:		printf(" P "); break;
			case -CAVALO:	printf(" C "); break;
			case -BISPO:	printf(" B "); break;
...
void lance2movi(char *m,int *l, int flag_50, int espec)  //int para char
{
	int i;
...
//		n[2]='x';
	...
	switch(espec)    //promocao para 4,5,6,7 == D,C,T,B
	{
		case 4: //n[5]='=';n[6]='D';n[7]='\0';     //promoveu a Dama
        		m[4]='q';m[5]='\0';
				break;
		case 5: //n[5]='=';n[6]='C';n[7]='\0';     //promoveu a Cavalo
        		m[4]='n';m[5]='\0';
				break;
		case 6: //n[5]='=';n[6]='T';n[7]='\0';     //promoveu a Torre
        		m[4]='r';m[5]='\0';
				break;
		case 7: //n[5]='=';n[6]='B';n[7]='\0';     //promoveu a Bispo
        		m[4]='b';m[5]='\0';
				break;
	}
}

int movi2lance(int *l,char *m)  //transforma char de entrada em int
{
	int i;
					...
					for(col=i-1;col<=i+1;col++)		//Rei anda normalmente
						for(lin=j-1;lin<=j+1;lin++)
						{
							...
							if(!xeque_rei_das(tabu.vez,tabaux))
							{
								...
								pmoviant=pmovi;
								pmovi=pmovi->prox; pmovi->prox=NULL;
							}
						}
					...
					if(tabu.vez==brancas)
					{
						if(j+1<8)
						{
							if(tabu.tab[i][j+1]==VAZIA)
							{
								...
								{
									k=4;	//4:dama, 5:cavalo, 6:torre, 7:bispo
	promoveb:
									pmovi->lance[0]=i;
									pmovi->lance[1]=j;
									pmovi->lance[2]=i;
					...
					if(tabu.vez==brancas) //vez das brancas
					{
						...
						do		//diagonal esquerda e direita do peao.
						{
							if(tabu.tab[k][j+1]>0)	//peca preta
							{
								...
								if(!xeque_rei_das(brancas,tabaux))
								{
									l=4;    	//4:dama, 5:cavalo, 6:torre, 7:bispo
				...
				case BISPO:
					...
					if(peca!=BISPO)
					{
						for(k=-1;k<2;k+=2)   //k= -1, 1
						{
							...
							l=0;m=0;    //m=idem para a vertical
							do
							{
...
//				getchar();
            ...
            {
            	if(debug)
				    fprintf(fsaida,"tellopponent Xadreco v.%.1f para XBoard/WinBoard, baseado no Algoritmo Minimax, por Ruben Carlo Benante, 04/06/04.\n",version);
			    printf("tellopponent Xadreco v.%.1f para XBoard/WinBoard, baseado no Algoritmo Minimax, por Ruben Carlo Benante, 04/06/04.\n",version);
            }
			tente=1;
			continue;
		}
...
//		if(!strcmp(movinito,"computer") && xboard) //nao esta funcionando xadreco versus xadreco
//        {
//            primeiro='c';
//            segundo='c';
//            pausa='n';
...
//		if(!strcmp(movinito,"1-0")) 	//vitoria das brancas
//			return('V');
//		if(!strcmp(movinito,"0-1")) 	//vitoria das pretas
//			return('v');
//		if(!strcmp(movinito,"1/2-1/2")) //empate
//			return('j');
        ...
        if(!strcmp(movinito,"setboard")) //funciona no prompt também
        {
			...
			for(j=7;j>=0;j--)
        		for(i=0;i<8;i++)
                    tabu->tab[i][j]=VAZIA; //a8, b8, ..., h8, a7, ..., a1, ..., h1

            scanf("%s",movinito); //Posição FEN
    	    if(debug)
            	fprintf(fsaida,"winboard: %s\n",movinito);
            j=7; //linha de '8' a '1'
            i=0; //coluna de 'a' a 'h'
            ...
            while(movinito[k]!='\0')
            {
                ...
                {
                    case 'K': tabu->tab[i][j]=-REI; i++; break;
                    case 'k': tabu->tab[i][j]=REI; i++; break;
                    case 'Q': tabu->tab[i][j]=-DAMA; i++; break;
                    case 'q': tabu->tab[i][j]=DAMA; i++; break;
                    case 'R': tabu->tab[i][j]=-TORRE; i++; break;
                    case 'r': tabu->tab[i][j]=TORRE; i++; break;
                    case 'B': tabu->tab[i][j]=-BISPO; i++; break;
                    case 'b': tabu->tab[i][j]=BISPO; i++; break;
                    case 'N': tabu->tab[i][j]=-CAVALO; i++; break;
                    case 'n': tabu->tab[i][j]=CAVALO; i++; break;
                    case 'P': tabu->tab[i][j]=-PEAO; i++; break;
                    case 'p': tabu->tab[i][j]=PEAO; i++; break;
                    case '/': i=0; j--; break;
            ...
            //int roqueb, roquep;
            //1:pode para os 2 lados.
            //0:nao pode mais.
            //3:mexeu TD. So pode K
            //2:mexeu TR. So pode Q
        	...
        	switch(res)
        	{
        		...
        		case '5':                           //50 lances
        		case 'r': tabu->situa=1; break;     //repeticao
        		case 'x': tabu->situa=2; break;		//xeque
        		case 'M': tabu->situa=3; break;     //Brancas perderam por Mate
        		case 'm': tabu->situa=4; break;     //Pretas perderam por mate
        		case 'T': tabu->situa=5; break;     //Brancas perderam por Tempo
        		case 't': tabu->situa=6; break;		//Pretas perderam  por tempo
        		default: tabu->situa=0;				//nada...
        	}
...
//				getchar();
	...
	if(pval->especial>3)  //promocao do peao 4,5,6,7: Dama,Cavalo, Torre, Bispo
	{
    	if(xboard)
        {
			...
			{
				case 'q': pval->especial=4; break; //dama
				case 'r': pval->especial=6; break; //torre
				case 'b': pval->especial=7; break; //bispo
				case 'n': pval->especial=5; break; //cavalo
				default : pval->especial=4; break; //dama
			}
        }
        else
        {
...
//				scanf("%c",&peca); //peca=getche();
			switch(peca)
			{
				...
				default : printf("Erro na função pega(\"dtcb\",\"Promove?\")");
...
int igual(int *l1, int *l2)	//dois lances[4] sao iguais
{
	int j;
	for(j=0;j<4;j++)
		if(l1[j]!=l2[j])
	...
	for(i=0;i<8;i++)				//insuficiencia de material
		for(j=0;j<8;j++)
		{
			switch(tabu.tab[i][j])
			{
				case -DAMA:
				case -TORRE:
				case -PEAO:			insuf_branca+=3;
												break;
				case DAMA:
				case TORRE:
				case PEAO:			insuf_preta+=3;
												break;
				case -BISPO:		insuf_branca+=2;
												break;
				case BISPO:			insuf_preta+=2;
												break;
				case -CAVALO:		insuf_branca++;
												break;
				case CAVALO:		insuf_preta++;
												break;
			}
...
//						scanf("%c",&com); //com=getche();
    ...
    clock1=clock(); //clock_t é long int %ld
	...
	if(!USALIVRO)
    {
        ...
        while(true) //for(nv=1; nv<=7; nv++) //funcao COMPJOGA
        {
            ...
            clock2=clock();
            difclock=clock2-clock1;
            if(difclock>tempomov) // termino do laço infinito baseado no tempo
                break;
            nv++;
        }
        libera_lances(result.plance);
...
char analisa(tabuleiro *tabu)
{
    tabuleiro tanalise;
    char res, m[80];
    int nv, melhorvalor1; // lances calc. em maior nivel tem mais importancia?
    movimento *melhorcaminho1, *loop;
    ...
    melhorcaminho1=NULL;
//  melhornv1=1;
//  nv=nivel; //quebra-galho temporário para funcionar sem o tempo.
//    for(nv=1; nv<=nivel; nv++)
//mudou para busca em amplitude: variavel nivel obsoleta!
    nv=1;
    while(true) //for(nv=1; nv<=7; nv++) //funcao ANALISA
    {
        // tabuleiro atual, profundidade zero, limite maximo de estatico (beta ou uso), limite minimo de estatico (alfa ou passo), nivel da busca
    	minimax(*tabu,0,10001,-10001, nv);  //retorna o melhor caminho a partir de tab...
...
void minimax(tabuleiro atual,int prof, int uso, int passo, int niv)
{
	...
	if(!xboard && 1==0)
    {
		...
		switch(gira)
		{
			case (unsigned char)1: printf("("); break;
			case (unsigned char)61: printf("|"); break;
			case (unsigned char)121: printf(")"); break;
			case (unsigned char)181: printf("|");break;
		}
    }
	succ=NULL;
	melhor_caminho=NULL;
	...
	if(profsuf(prof, niv)) // profundidade suficiente ==1 ou ==-1:tempo estourou
	{
	...
	{
		result.plance=NULL;
		result.valor=estatico(atual,1);	//entao o estatico refletira isso: "pat"
		return;
	}
	cabeca_succ=succ;
	while(succ!=NULL)				//laco para analisar todos sucessores
	{
		...
		switch(tab.situa)
		{
			...
			case 2: profflag=2; break;			//2:Xeque!  Liberou
			default: profflag=0;					//1:Empate, 3,4:Mate ou 5,6:Tempo. Nao pode passar o nivel
		}
        ...
        if(passo>=uso) //corte alfa-beta! Isso está certo? Não é passo<=uso? Conferir.
            break;
		succ=succ->prox;
	}
	result.valor=passo;
...
char joga_em(tabuleiro *tabu, movimento movi, int cod)
{
	...
	if(tabu->vez==brancas)
		...
		{
			case 0:	tabu->roqueb=0; break;		//mexeu o rei
			case 1: break;                    //nao fez lance que interfere o roque
			case 2: if(tabu->roqueb==1 ||tabu->roqueb==2) //mexeu torre do rei
								tabu->roqueb=2;
							else
								tabu->roqueb=0;
							break;
			case 3: if(tabu->roqueb==1 ||tabu->roqueb==3) //mexeu torre da dama
								tabu->roqueb=3;
							else
								tabu->roqueb=0;
							break;
		...
		switch(movi.roque) //avalia o que este lance causa para futuros roques
		{
			case 0:	tabu->roquep=0; break;		//mexeu o rei
			case 1: break;                    //nao fez lance que interfere o roque
			case 2: if(tabu->roquep==1 ||tabu->roquep==2) //mexeu torre do rei
								tabu->roquep=2;
							else
								tabu->roquep=0;
							break;
			case 3: if(tabu->roquep==1 ||tabu->roquep==3) //mexeu torre da dama
								tabu->roquep=3;
							else
								tabu->roquep=0;
							break;
...
//copia pmovi no comeco da lista *plan, e retorna a cabeca desta NOVA lista (mel)
...
void copimov(movimento *dest, movimento *font) //nao compia o ponteiro prox
{
	int i;
...
movimento *copilistmov(movimento *font) //cria nova lista de movimentos para destino
{
...
int estatico(tabuleiro tabu, int cod)
{
	int totb=0, totp=0;
	int i,j, cor, peca;
	...
	for(i=0;i<8;i++)
		for(j=0;j<8;j++)
		{
			...
			if(peca<0) //se peca branca:
            {
				...
				if(tabu.vez==brancas || (qtdb>=qtdp && -peca<=menorp) || menorp==0) //vez das brancas ou defesab>=ataquep
    				totb+=abs(peca);  //pecas brancas: total=total + material. So soma se nao estiver pendurada!
            }
			else						//se peca preta:
            {
                qtdp=qataca(pretas,i,j,tabu,&menorp);
                qtdb=qataca(brancas,i,j,tabu,&menorb);
				if(tabu.vez==pretas || (qtdp>=qtdb && peca<=menorb) || menorb==0) //vez das pretas ou defesap>=ataqueb
    				totp+=peca;		//pecas pretas: total=total + material. So soma se nao estiver pendurada!
            }
		}
	...
	for(i=0;i<8;i++)
		for(j=0;j<8;j++)
		{
			...
			{
				if(i>2)						//faltando 4 casas ou menos para promover ganha +1
					totb+=1;
				if(i>3)						//faltando 3 casas ou menos para promover ganha +1+1
					totb+=1;
				if(i>4)
					totb+=3;					//faltando 2 casas ou menos para promover ganha +1+1+3
				if(i>5)
					totb+=3;					//faltando 1 casas ou menos para promover ganha +1+1+3+3
			}
			if(tabu.tab[i][j]==PEAO)
			{
				if(i<5)
					totp+=1;					//faltando 4 casas ou menos para promover ganha +1
				if(i<4)
					totp+=1;					//faltando 3 casas ou menos para promover ganha +1+1
				if(i<3)
					totp+=3;					//faltando 2 casas ou menos para promover ganha +1+1+3
				if(i<2)
					totp+=3;					//faltando 1 casas ou menos para promover ganha +1+1+3+3
			}
		}
	...
	{                                                   //  0  |   isob = 0        | isob = 1
		...
		switch(isob)
		{
			case 0:	if(!peaob)					//se nao achou, peaob==0
							isob++; 		//isob=1, se achou isob continua 0
						break;
			case 1:	if(peaob)						//se achou, peaob !=0
							isob++;     //isob=2, se nao achou isob continua 1
						break;
			case 2:	if(peaob)						//se achou...
		...
		if(isob==3)
		{
			totb-=6;	//penalidade para peao isolado
			isob=1;
		}
		switch(isop)
		{
			case 0:	if(!peaop)					//se nao achou, peaop==0
							isop++;
						break;
			case 1:	if(peaop)						//se achou, peaop !=0
								isop++;
						break;
			case 2:	if(peaop)						//se achou...
							isop=0;
						else								//senao...
		...
		if(isop==3)
		{
			totp-=6;		//penalidade para peao isolado
			isop=1;
		}
	}
...
//    if(cod==-1) //avaliando posicao estatica de nivel baixo, pois tempo estourou!
//        if(tabu.vez==brancas)
//            totb-=2*DAMA;
//        else
//            totp-=2*DAMA;
...
void volta_lance(tabuleiro *tabu) //para voltar um lance
{
    ...
    if(tabu->numero<25 && setboard<1) //nao usar abertura em posições de setboard
        USALIVRO=1;
}
...
{		//retorna o numero de ataques que a "cor" faz na casa(col,lin)
        ...
        // codigo abaixo nao utilizado.
		//quem= 0:ninguem, +1:P, +2:BouC, +4:T, +8:D, +16:R
		// 3:BouC,P; 5:T,P; 6:T,BouC; 7:T,BouC,P; 9:D,P; 10:D,BouC; 11:D,BouC,P;
